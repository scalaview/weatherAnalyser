...
def printResults(items, rules):
    ...
    for item, support in items:
        print "item: %s , %.3f" % (item[0], support)
    print "\n------------------------ RULES:"
    for rule, confidence in rules:
        pre, post = rule
        print "Rule: %s ==> %s , %.3f" % (pre[0], post[0], confidence)
...
if __name__ == "__main__":
    ...
    optparser.add_option('-s', '--minSupport', dest='minS', help = 'minimum support value', default=0.05, type='float')
    optparser.add_option('-c','--minConfidence', dest='minC', help = 'minimum confidence value', default = 0.06, type='float')
